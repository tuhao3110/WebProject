...
class TicketSuggestion_show extends Controller
{
    //
...
//        die(var_dump($similarmatrix));
		...
		{
			$vitrighe = DB::table('ve')->where('Mã_chuyến_xe','=',$machuyenxe)->select('Vị_trí_ghế')->get();
			return view('ticket', ['vitrighe' => $vitrighe,'similarmatrix' => $similarmatrix,'matrix' => $matrix,'normmatrix' => $normmatrix,'normmatrixFull' => $normmatrixFull,'matrixFinal' => $matrixFinal]);
		}
...
//                    self::hoandoi($bangdenghi[$i],$bangdenghi[$j]);
		...
		return view('ticket', ['kq' => $kq,'vitrighe' => $vitrighe,'similarmatrix' => $similarmatrix,'matrix' => $matrix,'normmatrix' => $normmatrix,'normmatrixFull' => $normmatrixFull,'matrixFinal' => $matrixFinal]);
